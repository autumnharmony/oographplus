...
//        Injector injector = Guice.createInjector(Modules.combine(new CommonsModule(),new CodeGeneratorModule()));
    ...
    public Graph walk(Set<XShape> all) {
        ...
        Set<XShape> notConnectors = Sets.newHashSet();
        for (XShape shape : all) {
            if (shapeHelperWrapper.isConnectorShape(shape)) {
                ...
                if (!shapeHelperWrapper.isTextShape(shape)) {
                    notConnectors.add(shape);
                }
            }
        }
        XShape current;
        ...
        for (XShape shape : notConnectors) {
            Node node = nodeFactory.create(shape);
            nodes.add(node);
        }
