...
public final class Interceptor extends ComponentBase
        implements XDispatchProviderInterceptor, XInterceptorInfo, XDispatch, XStatusListener
{
    ...
    public Interceptor(XComponentContext context, XFrame xFrame) {
        ...
        if (m_aURL == null) {
            ...
            try {
                URL[] aURLs = { m_aURL };
                XURLTransformer xTransformer = (XURLTransformer)UnoRuntime.queryInterface(XURLTransformer.class, this.m_xContext.getServiceManager().createInstanceWithContext("com.sun.star.util.URLTransformer", this.m_xContext));
        ...
        try {
            this.m_xMsgBoxFactory = ((XMessageBoxFactory)UnoRuntime.queryInterface(XMessageBoxFactory.class, this.m_xContext.getServiceManager().createInstanceWithContext("com.sun.star.awt.Toolkit", this.m_xContext)));
        }
        ...
        XDispatchProviderInterception xInterception = (XDispatchProviderInterception)UnoRuntime.queryInterface(XDispatchProviderInterception.class, xFrame);
        ...
        XDispatchProvider xProvider = (XDispatchProvider)UnoRuntime.queryInterface(XDispatchProvider.class, this.m_xFrame);
    ...
    public synchronized void stopIntercepting() {
        ...
        try
        {
            XDispatchProviderInterception xInterception = (XDispatchProviderInterception)UnoRuntime.queryInterface(XDispatchProviderInterception.class, this.m_xFrame);
    ...
    private void showMessageBox(String sCommand) {
        try {
            XWindowPeer xWindowPeer = (XWindowPeer)UnoRuntime.queryInterface(XWindowPeer.class, this.m_xFrame.getContainerWindow());
